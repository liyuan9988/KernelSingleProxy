...
from src.data.data_class import SingleProxyTrainDataSet, SingleProxyTestDataSet, SingleProxyTrainDataSetTorch, \
    SingleProxyTestDataSetTorch, split_train_data
...
class CEVAETrainer(object):
    ...
    def train(self, train_data: SingleProxyTrainDataSet, test_data_t, verbose: int = 0):
        train_data, val_data = split_train_data(train_data, 0.9)
        train_data_t = SingleProxyTrainDataSetTorch.from_numpy(train_data)
        val_data_t = SingleProxyTrainDataSetTorch.from_numpy(val_data)
        ...
        for t in range(self.n_epoch):
            self.opt.zero_grad()
            loss = self.distribution(proxy=train_data_t.proxy,
            ...
            with torch.no_grad():
                val_loss = self.distribution(proxy=val_data_t.proxy,
            ...
            if verbose >= 2 and t % 10 == 0:
                with torch.no_grad():
                    ...
                    mdl.fit(train_data_t.proxy, train_data_t.treatment, train_data_t.outcome,
                            10)
            ...
            if verbose >= 2 and t % 10 == 0:
                with torch.no_grad():
                    logger.info(self.distribution.q_z_xty(val_data_t.proxy,
                                                          val_data_t.treatment,
                                                          val_data_t.outcome).mean)

        mdl = CEVAEModel(self.distribution)
        mdl.fit(train_data_t.proxy, train_data_t.treatment, train_data_t.outcome, 10)
        return mdl
...
def cevae_experiments(data_config: Dict[str, Any], model_param: Dict[str, Any],
    ...
    test_data_t = SingleProxyTestDataSetTorch.from_numpy(test_data)
    if trainer.gpu_flg:
    ...
    if test_data.structural is not None:
        oos_loss: float = np.mean((pred - test_data.structural) ** 2)
