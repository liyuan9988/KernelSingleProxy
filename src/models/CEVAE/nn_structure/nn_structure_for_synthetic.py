...
class SyntheticDistribution(AbstractDistribution):
    ...
    def __init__(self, n_hidden_dim: int = 20, n_learning_sample: int = 10):
        super(SyntheticDistribution, self).__init__(n_hidden_dim, n_learning_sample)
    ...
    def build_p_x_z(self):
        feature = nn.Sequential(nn.Linear(self.n_hidden_dim, 16),
                                nn.ReLU())
        self.pxz_mean_net = nn.Sequential(feature, nn.Linear(16, 1))
        self.pxz_sigma_net = nn.Sequential(feature, nn.Linear(16, 1), nn.Softplus())
    ...
    def build_p_y_tz(self):
        feature = nn.Sequential(nn.Linear(self.n_hidden_dim + 1, 32),
                                nn.ReLU(),
                                nn.Linear(32, 16),
                                nn.ReLU())
    ...
    def build_q_z_xty(self):
        feature = nn.Sequential(nn.Linear(3, 32),
                                nn.ReLU(),
                                nn.Linear(32, 16),
                                nn.ReLU())
        self.qzxty_mean_net = nn.Sequential(feature, nn.Linear(16, self.n_hidden_dim))
        self.qzxty_sigma_net = nn.Sequential(feature, nn.Linear(16, self.n_hidden_dim), nn.Softplus())
