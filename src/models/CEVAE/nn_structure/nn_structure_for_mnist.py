...
class MNISTDistribution(AbstractDistribution):
    ...
    def __init__(self, n_hidden_dim: int = 20, n_learning_sample: int = 10):
        super(MNISTDistribution, self).__init__(n_hidden_dim, n_learning_sample)
    ...
    def p_x_z(self, z):
        return Normal(loc=self.pxz_mean_net(z),
                      scale=torch.clip(self.ptz_sigma_net(z), min = 0.01))
    ...
    def build_p_x_z(self):
        feature = nn.Sequential(nn.Linear(self.n_hidden_dim, 32),
                                nn.ReLU(),
                                nn.Linear(32, 128),
                                nn.ReLU(),
                                nn.Linear(128, 256),
        ...
        self.pxz_mean_net = nn.Sequential(feature,
                                          nn.Linear(256, 784),
                                          )
        self.pxz_sigma_net = nn.Sequential(feature,
                                           nn.Linear(256, 784),
                                           nn.Softplus())
    ...
    def build_q_z_xty(self):
        feature = nn.Sequential(nn.Linear(784 + 2, 256),
                                nn.ReLU(),
                                nn.Linear(256, 128),
                                nn.ReLU(),
                                nn.Linear(128, 32),
